...
public class Board extends JPanel {
	...
	public Board(String player, boolean iscomputer, boolean isplayerfirst) {
		...
		alfabeta = new AlfaBeta(this);
		computer = new Computer(this, alfabeta);
		if (isComputer) {
			computer.start();
	...
	public void generalTestWinner() {
		if (isComputer) {
			if (currentState.checkWinner(playLeft) == -1) {
				displayEndingMessage("Winner", "Player wins!");
			} else if (currentState.checkWinner(playLeft) == 0) {
				displayEndingMessage("Draw!", "You draw!");
			} else if (currentState.checkWinner(playLeft) == 1) {
